...
function truthTable() {
  const pad = x => x.toString().padStart(5);
  const bits = x => (x | 0x80).toString(2).slice(-4);

  console.log(pad('S'), pad('M'), pad('A'), pad('B'),
              pad('F[3]'), pad('F[2]'), pad('F[1]'), pad('F[0]'));
...
if (true) {
  f.forEach((fv, x) => console.log(`  assign F[${x}] = ${fv};\n`));
  console.log('  assign CG =', g, ';\n');
  console.log('  assign CP =', p, ';\n');
  console.log('  assign COUT =', c4, ';\n');
}
